...
<script lang="ts">
    ...
    const additionalSettings = {
        onChange: new Function("value", "text", "$choice", `document.dispatchEvent( new CustomEvent("` + channelname + `", { detail: value } ) );`)
    };
    // ----------------------------------------------------------------------------------------------------------------------------------------------
...
{#if multiple}
    <div {id} class={classString(ui, $$restProps, "multiple dropdown")} data-selected={selected} data-module={rationalize([serialize("dropdown", settings={...settings, ...additionalSettings}), serialize((popup?"popup":null), (typeof(popup) === "boolean")?undefined:popup)])} {...otherProps($$restProps)} on:change={doChange}>
        <slot />
    </div>
{:else}
