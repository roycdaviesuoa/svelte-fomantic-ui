...
<div role="link" tabindex={0} {id} class={classString(ui, $$restProps, "menu")} data-module={rationalize([serialize((popup?"popup":null), (typeof(popup) === "boolean")?undefined:popup), serialize("menu", {...functionize(ID, id, callbacks), ...settings})])} {...otherProps($$restProps)} on:click={setSelected} on:change>
    <slot />
</div>
