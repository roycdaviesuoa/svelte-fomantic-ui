...
{:else}
    <div role="link" tabindex={0} class={classString(ui, $$restProps, "image")} data-module={serialize((popup?"popup":null), (typeof(popup) === "boolean")?undefined:popup)} {...otherProps($$restProps)} on:mouseenter={()=>{mouseover=true;}} on:mouseleave={()=>{mouseover=false;}} on:focus>
        <slot {mouseover}/>
    </div>
{/if}