...
<div {id} class={classString(ui, $$restProps, "sticky")} data-module={rationalize([serialize("sticky", {...functionize(ID, id, callbacks), ...settings}), serialize((popup?"popup":null), (typeof(popup) === "boolean")?undefined:popup)])} {...otherProps($$restProps)}>
    <slot />
</div>