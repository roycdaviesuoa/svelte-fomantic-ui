...
<div {id} class={classString(ui, $$restProps, "rating")} data-module={rationalize([serialize("rating", {...functionize(ID, id, callbacks), ...settings}), serialize((popup?"popup":null), (typeof(popup) === "boolean")?undefined:popup)])} {...otherProps($$restProps)}>
    <slot />
</div>